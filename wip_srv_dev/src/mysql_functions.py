...
class MySqlDb:
  ...
  #####################
  # Update advice in WIP score if adviced
  #####################  
  def update_wip_score_advice(self, doc_id):
    with self.conn.cursor(dictionary=True) as cur:
      print('update advice in wip_score')
      wip_tbl_score = os.getenv('MYSQL_SCORES')
      sql = f"""
      UPDATE {wip_tbl_score} SET adviced = 1 WHERE doc_id = '{ doc_id }';
      """
      cur.execute(sql)
      self.conn.commit()
