...
def msg_processor_sym(chat_sys, msg_org):
  ...
  with open('wip_params/ai_params_5w2h.yaml') as file:
    params = yaml.safe_load(file.read())
  ai_response_org = ai_5w2h.ask_ChatCompletion(ai_5w2h.prompt_generator(msg_in, params['AI_PROMPT_S'], params['AI_PROMPT_M']), params['AI_MODEL'], params['AI_MAX_TOKEN'], params['AI_N'], params['AI_STOP'], params['AI_TEMPERATURE'])
  print("AI_RESPONSE: ", ai_response_org)
  print("######### AI Response ##########")


  if len(ai_response_org) == 0:
      print("api.py: ERROR: AI response is empty!")
  else:
      ########## store Message with Attributes into MongoDB
      ai_response = ai_msg_processor.ai_msg_parser(msg_in['domain_id'], msg_in['user_id'], msg_in['chat_sys'], msg_in['display_name'], msg_in['chat_user_id'], msg_in['conversation_id'], "", msg_in['message_id'], msg_in['date'], msg_in['time'], ai_response_org, msg_in['message'], msg_in['mentioned_user_id'])
      ...
      try:
          ...
          print("chat_msg_processor_sym.py: ERROR: cannot insert score into MySQL: %s", e)
...
def msg_formatter_sym(user_id, domain_id, chat_sys, origin, msg_decoded, msg_notag):
  ...
  mentioned_user_id = msg_decoded.get('mentioned_user_id')
  ...
  msg_json = {'domain_id': domain_id, 'user_id': user_id, 'chat_sys': chat_sys, 'origin': origin, 'display_name': display_name, 'chat_user_id': chat_user_id, 'conversation_id': conversation_id, 'thread_id': thread_id, 'message_id': message_id, 'date': date, 'time': time, 'message': msg_notag, 'mentioned_user_id': mentioned_user_id}
  return msg_json
